...
if (isset($_POST['id']) && isset($_POST['category_id']) && isset($_POST['name']) && isset($_POST['description']) )
    {
		...
		$sql_command_check="SELECT *FROM $tablename WHERE categories = '$category'";
		...
		else
		{
			...
			$sqlcommand_update_name="UPDATE $tablename SET categories='$category' WHERE id='$category_id'";
			$result_update_name = mysqli_query($db::$connection,$sqlcommand_update_name) ;
			if(!$result_update_name)
			{
				// failed to insert column
				$response["success"] = -1;
				$response["message"] = " failed to update category name ". mysqli_error($db::$connection);
				echo json_encode($response);
			}
			
		}
		//update the description
		$sqlcommand_update_des="UPDATE $tablename SET descriptions='$description' , datechanged='$today' WHERE id='$category_id'";
		$result_update_des = mysqli_query($db::$connection,$sqlcommand_update_des) ;
		if(!$result_update_des)
		{
			// failed to insert column
			$response["success"] = -2;
			$response["message"] = "failed to update category description ". mysqli_error($db::$connection);
			echo json_encode($response);
		}
		$response["success"] = 1;
