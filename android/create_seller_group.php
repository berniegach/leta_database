...
if (isset($_POST['id']) && isset($_POST['name']) && isset($_POST['category_id']))
    {
		...
		$sql_command_check="SELECT *FROM $tablename WHERE categories = '$category_id' AND groups = '$name'";
		$result_check = mysqli_query($db::$connection,$sql_command_check) ;
		if(!$result_check || mysqli_num_rows($result_check)>0)
		{
			// failed to insert column
			$response["success"] = -2;
			$response["message"] = "group already there.". mysqli_error($db::$connection);
			echo json_encode($response);
		}
		else
		{
			// mysql inserting a new row
			$sql_command="INSERT INTO $tablename(categories,groups, dateadded) VALUES('$category_id','$name','$today')";
