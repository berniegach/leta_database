...
if (isset($_POST['id']) && isset($_POST['category_id']) && isset($_POST['group_id']) && isset($_POST['name']) && isset($_POST['description']) )
    {
		...
		$sql_command_check="SELECT *FROM $tablename WHERE categories = '$category_id' AND groups = '$group'";
		...
		else
		{
			...
			$sqlcommand_update_name="UPDATE $tablename SET groups='$group' WHERE id='$group_id'";
			$result_update_name = mysqli_query($db::$connection,$sqlcommand_update_name) ;
			if(!$result_update_name)
			{
				// failed to insert column
				$response["success"] = -1;
				$response["message"] = " failed to update group name ". mysqli_error($db::$connection);
				echo json_encode($response);
			}
			
		}
		//update the description
		$sqlcommand_update_des="UPDATE $tablename SET descriptions='$description' , datechanged='$today' WHERE id='$group_id'";
		$result_update_des = mysqli_query($db::$connection,$sqlcommand_update_des) ;
		if(!$result_update_des)
		{
			// failed to insert column
			$response["success"] = -2;
			$response["message"] = "failed to update group description ". mysqli_error($db::$connection);
			echo json_encode($response);
		}
		$response["success"] = 1;
