...
if (isset($_POST["email"]) &&isset($_POST["password"]))
    ...
    // get an account
    $sql_command="SELECT *FROM sellers_accounts WHERE emails = '$email'";
    ...
    if (!empty($result))
        ...
        if (mysqli_num_rows($result) > 0) 
            ...
            if(password_verify($password, $hashed_password))
            {
				...
				$account['order_radius']=$result['orderrange'];
				$account['order_format']=$result['orderformat'];
				$account['number_of_tables']=$result['numberoftables'];
                $account['dateadded']=$result['dateadded'];
                $account['datechanged']=$result['datechanged'];
				$account['today']=date("d-m-Y H:i");
                //response
                $response["success"] = 1;
